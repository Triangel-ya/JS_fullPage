...
function initial(){
	...
	Slide.prototype.move=function(speed,time,dir){  
		if(lock == 0){
			lock = 1;
			setTimeout(function(){
				setTime(speed,time,dir);
			},time);
			index = index + dir;
		}	
	}
	function setTime(speed,time,dir){//speed:移动速度；time:每次移动时间；dir:决定左右移动，左为-1，右为1
		if(index == slidePage.length){
			slideContainer.appendChild(slidePage[0]);
			index --;
			slideContainer.style.left = -(index-1) * cWidth + "px";
		...
		else if(Math.abs(Math.abs(slideContainer.offsetLeft) - index * cWidth) > 0  && Math.abs(index*cWidth-Math.abs(slideContainer.offsetLeft)) >= Math.abs(speed)){
			slideContainer.style.left = slideContainer.offsetLeft + speed + "px";
		}
		else{
			slideContainer.style.left = slideContainer.offsetLeft + (Math.abs(slideContainer.offsetLeft) - index*cWidth) + "px";
			lock = 0;
			return;
		}
		setTimeout(function(){
